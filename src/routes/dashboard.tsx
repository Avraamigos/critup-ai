...
export function DashboardPage() {
  ...
  return (
    <div style={{ padding: '28px', maxWidth: 1080, position: 'relative' }}>
      ...
      <div style={{ marginBottom: 28, position: 'relative', zIndex: 1 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 3 }}>
          <h1 style={{ fontSize: 24, fontWeight: 700, letterSpacing: '-0.025em', margin: 0, color: c.textPrimary, fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Display', 'Helvetica Neue', 'Inter', sans-serif" }}>
            {greeting}, Alex
          </h1>
          <div style={{ padding: '3px 10px', borderRadius: 100, fontSize: 11, fontWeight: 600, background: 'oklch(0.72 0.18 45 / 0.1)', color: '#F97316', border: '1px solid oklch(0.72 0.18 45 / 0.25)' }}>
      ...
      {/* Top row */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 18, marginBottom: 20, position: 'relative', zIndex: 1 }}>
        {/* Active project */}
        <div style={{ gridColumn: '1 / 3', background: c.cardBg, borderRadius: 18, padding: '24px', border: `1px solid ${c.border}` }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
            <div>
              ...
              <h2 style={{ fontSize: 17, fontWeight: 700, letterSpacing: '-0.02em', margin: '0 0 6px', color: c.textPrimary, fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Display', 'Helvetica Neue', 'Inter', sans-serif" }}>
                {active.name}
              </h2>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <div style={{ padding: '3px 9px', borderRadius: 100, fontSize: 10, fontWeight: 600, background: 'oklch(0.72 0.18 45 / 0.1)', color: '#F97316', border: '1px solid oklch(0.72 0.18 45 / 0.25)' }}>
                  {active.stage}
                </div>
            ...
            <Link to="/analysis/$projectId" params={{ projectId: active.id }} style={{
              padding: '8px 16px', borderRadius: 100, background: '#F97316', border: 'none', color: '#fff',
              ...
              <Eye size={13} color="#fff" /> View analysis
            </Link>
          </div>
          <div style={{ display: 'flex', gap: 20 }}>
            <ScoreRing score={active.scores.concept} label="Concept" size={82} theme={theme} />
            <ScoreRing score={active.scores.spatial} label="Spatial" size={82} theme={theme} />
            <ScoreRing score={active.scores.presentation} label="Presentation" size={82} theme={theme} />
          </div>
        </div>

        {/* New project */}
        <Link to="/projects/new" style={{
          background: c.cardBg, borderRadius: 18, padding: '24px', border: `1px solid ${c.border}`,
          cursor: 'pointer', transition: 'all 0.2s', position: 'relative', overflow: 'hidden',
          display: 'flex', flexDirection: 'column', justifyContent: 'space-between', textDecoration: 'none',
        }}
        ...
        >
          ...
          <div>
            <div style={{ width: 40, height: 40, borderRadius: 11, background: 'oklch(0.72 0.18 45 / 0.1)', border: '1px solid oklch(0.72 0.18 45 / 0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 14 }}>
              <Plus size={18} color="#F97316" />
            </div>
            <h3 style={{ fontSize: 15, fontWeight: 700, letterSpacing: '-0.01em', margin: '0 0 6px', color: c.textPrimary, fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Display', 'Helvetica Neue', 'Inter', sans-serif" }}>New project</h3>
            <p style={{ fontSize: 12, color: c.textMuted, lineHeight: 1.5, margin: 0 }}>Upload your brief and get stage-specific feedback</p>
          </div>
          <div style={{ fontSize: 13, color: '#F97316', fontWeight: 600, marginTop: 18, display: 'flex', alignItems: 'center', gap: 4 }}>
      ...
      {/* Quick actions */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, marginBottom: 28, position: 'relative', zIndex: 1 }}>
        {[
          ...
          <Link key={label} to={to} style={{
            background: c.cardBg, borderRadius: 14, padding: '16px 18px', border: `1px solid ${c.border}`,
          ...
          >
            ...
            <div>
              <div style={{ fontSize: 13, fontWeight: 600, color: c.textPrimary, marginBottom: 1 }}>{label}</div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {MOCK_PROJECTS.slice(0, 3).map((p) => (
            <Link key={p.id} to="/analysis/$projectId" params={{ projectId: p.id }} style={{
              background: c.cardBg, borderRadius: 13, padding: '14px 18px', border: `1px solid ${c.border}`,
              display: 'flex', alignItems: 'center', gap: 14, cursor: 'pointer', transition: 'all 0.15s', textDecoration: 'none',
            }}
              onMouseEnter={e => e.currentTarget.style.borderColor = c.hoverBorder}
              onMouseLeave={e => e.currentTarget.style.borderColor = c.border}
            >
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: c.textPrimary, marginBottom: 4 }}>{p.name}</div>
                <div style={{ display: 'flex', gap: 7, alignItems: 'center' }}>
                  <div style={{ padding: '2px 8px', borderRadius: 100, fontSize: 10, fontWeight: 600, background: `${p.stageColor}18`, color: p.stageColor, border: `1px solid ${p.stageColor}40` }}>{p.stage}</div>
              ...
              <div style={{ display: 'flex', gap: 14 }}>
                {([['C', p.scores.concept], ['S', p.scores.spatial], ['P', p.scores.presentation]] as [string, number][]).map(([l, v]) => (
                  <div key={l} style={{ textAlign: 'center' }}>
                    <div style={{ fontSize: 15, fontWeight: 700, fontFamily: 'monospace', color: v >= 8 ? 'oklch(0.72 0.17 145)' : v >= 6 ? '#F97316' : 'oklch(0.65 0.18 25)' }}>{v}</div>
