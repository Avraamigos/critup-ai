...
import { ChevronLeft, Play, Pause, Download, Loader2, AlertCircle, Plus, Volume2, VolumeX, Upload, X, FileText, Link, Check, Users, Globe, Sparkles } from 'lucide-react'
    ...
    <div style={{ height: 'calc(100vh - 54px)', display: 'flex', flexDirection: 'column', overflow: 'hidden', fontFamily: "'Inter',sans-serif", background: c.bg }}>
      ...
      <div style={{ padding: isMobile ? '8px 14px' : '10px 22px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0, borderBottom: `1px solid ${c.border}` }}>
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          ...
          {/* Jump to the poster tool with this project pre-associated. Pro-gated
              like other AI actions; the tool tags the generation to this analysis. */}
          <button
            onClick={() => {
              if (!isPro) { navigate({ to: '/pricing' }); return }
              navigate({ to: '/tools/poster', search: { analysisId: latestAnalysis.id } })
            }}
            title={!isPro ? t('analysis.voiceProTitle') : undefined}
            style={{ display: 'flex', alignItems: 'center', gap: 6, padding: isMobile ? '7px 10px' : '7px 14px', borderRadius: 100, background: c.cardBg, border: `1px solid ${c.border}`, color: c.textMuted, fontSize: 12, cursor: 'pointer', transition: 'all 0.15s' }}
            onMouseEnter={e => { e.currentTarget.style.borderColor = '#F97316'; e.currentTarget.style.color = '#F97316' }}
            ...
            <div key="summary" style={{ animation: 'slide-up 0.4s ease-out', display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              {juryQuestions.length > 0 && (
                <div style={{ background: c.cardBg, borderRadius: 14, padding: '14px', border: `1px solid ${c.border}` }}>
                  ...
                  <button onClick={() => { try { localStorage.setItem('critup_last_analysis_id', latestAnalysis!.id) } catch { /* ignore */ } navigate({ to: '/jury' }) }}
                    style={{ display: 'inline-block', padding: '6px 14px', borderRadius: 100, background: 'oklch(0.72 0.18 45/0.12)', color: '#F97316', fontSize: 12, fontWeight: 600, border: 'none', cursor: 'pointer' }}>
