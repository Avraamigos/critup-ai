...
import { Plus, Loader2, Sparkles, RefreshCw, Lock, HelpCircle, MessageSquare, Lightbulb, Minimize2, RotateCcw, Download } from 'lucide-react'
...
export function JuryPage() {
  ...
  // Per-slide shortened text (client-side, ephemeral) + which slide is shortening.
  const [shortened,    setShortened]    = useState<Record<number, string>>({})
  ...
  // A new generation / level switch invalidates any client-side shortened text.
  useEffect(() => { setShortened({}); setShorteningIdx(null) }, [slides])

  // ── Shorten a single slide's script (cheap Haiku call) ──────────────────────
  const shortenSlide = async (i: number) => {
    ...
    } catch { /* ignore — leave original */ }
    finally { setShorteningIdx(null) }
  }
  const restoreSlide = (i: number) => setShortened(prev => { const n = { ...prev }; delete n[i]; return n })

  // ── Export the script as a printable document ───────────────────────────────
  const exportScript = () => {
    const esc = (s: string) => s.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
    const levelLabel = t(`jury.level${level[0].toUpperCase()}${level.slice(1)}` as const)
    const today = new Date().toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric' })
    const rows = slides.map((s, i) => `
      <section class="slide">
        <div class="snum">${t('jury.slideLabel', { n: i + 1 })}</div>
        <h2>${esc(s.slideTitle || '')}</h2>
        <p class="script">${esc(shortened[i] ?? s.script)}</p>
        ${s.why ? `<div class="why"><span>${esc(t('jury.whyFrame'))}</span> ${esc(s.why)}</div>` : ''}
      </section>`).join('')
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"/><title>Jury Prep — ${esc(projectName || 'Presentation Script')}</title>
<style>
  *{box-sizing:border-box} body{margin:0;font-family:-apple-system,BlinkMacSystemFont,'SF Pro Display','Helvetica Neue',Arial,sans-serif;color:#1a1a1a;background:#fff;padding:48px 56px;line-height:1.6}
  .head{border-bottom:3px solid #F97316;padding-bottom:18px;margin-bottom:32px}
  .brand{font-size:18px;font-weight:800;letter-spacing:-0.02em}.brand span{color:#F97316}
  h1{font-size:26px;font-weight:800;letter-spacing:-0.03em;margin:14px 0 6px}
  .meta{font-size:12px;color:#6b7280}
  .slide{padding:18px 0;border-bottom:1px solid #ececec;break-inside:avoid;page-break-inside:avoid}
  .snum{font-size:10px;font-weight:800;letter-spacing:0.12em;color:#F97316;text-transform:uppercase;margin-bottom:4px}
  h2{font-size:16px;font-weight:700;margin:0 0 8px}
  .script{font-size:14px;margin:0;white-space:pre-wrap}
  .why{margin-top:12px;padding:12px 14px;background:#fff7ed;border-left:3px solid #F97316;border-radius:0 8px 8px 0;font-size:12.5px;color:#444}
  .why span{display:block;font-size:9px;font-weight:800;letter-spacing:0.12em;color:#F97316;text-transform:uppercase;margin-bottom:4px}
  .foot{margin-top:32px;font-size:11px;color:#9ca3af;text-align:center}
  @page{margin:18mm}
</style></head>
<body>
  <div class="head">
    <div class="brand">Critup<span>.ai</span></div>
    <h1>${esc(projectName || 'Presentation Script')}</h1>
    <div class="meta">Jury Prep · Presentation script · ${esc(levelLabel)} level · ${esc(today)}</div>
  </div>
  ${rows}
  <div class="foot">Generated by Critup.ai — adjust the script to match your real design intent before presenting.</div>
</body></html>`
            ...
            {!scriptBusy && slides.length > 0 && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14, animation: 'fade-up 0.4s ease-out' }}>
                {slides.map((s, i) => (
                  <div key={i} style={{ background: c.cardBg, borderRadius: 16, border: `1px solid ${c.border}`, overflow: 'hidden' }}>
                    ...
                    <div style={{ padding: '14px 18px' }}>
                      <p style={{ fontSize: 13.5, color: c.textPrimary, margin: '0 0 10px', lineHeight: 1.7, whiteSpace: 'pre-wrap' }}>{shortened[i] ?? s.script}</p>
                      <button onClick={() => (shortened[i] !== undefined ? restoreSlide(i) : shortenSlide(i))} disabled={shorteningIdx !== null}
                        style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '5px 12px', borderRadius: 100, background: shortened[i] !== undefined ? 'oklch(0.72 0.18 45/0.1)' : 'transparent', border: `1px solid ${shortened[i] !== undefined ? 'oklch(0.72 0.18 45/0.35)' : c.border}`, color: shortened[i] !== undefined ? '#F97316' : c.textMuted, fontSize: 11.5, fontWeight: 600, cursor: shorteningIdx !== null ? 'default' : 'pointer' }}>
                        {shorteningIdx === i
                          ? <><Loader2 size={12} style={{ animation: 'spin 1s linear infinite' }} /> {t('jury.shortening')}</>
                          : shortened[i] !== undefined
                            ? <><RotateCcw size={12} /> {t('jury.showOriginal')}</>
                            : <><Minimize2 size={12} /> {t('jury.shorten')}</>}
                      </button>
                    </div>
                    {s.why && (
                ...
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 2 }}>
                  <button onClick={exportScript} style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '9px 20px', borderRadius: 100, background: '#F97316', border: 'none', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', boxShadow: '0 4px 16px oklch(0.72 0.18 45/0.3)' }}>
                    <Download size={14} /> {t('jury.exportScript')}
                  </button>
                  <button onClick={() => generate(true)} style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '9px 18px', borderRadius: 100, background: 'transparent', border: `1px solid ${c.border}`, color: c.textMuted, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
